...
public class MediaSaveService extends Service {
    ...
    public boolean isQueueFull() {
        Log.v(TAG, "#isQueueFull   mMemoryUse = " + mMemoryUse/(1024*1024)+ "M");
        return (mMemoryUse >= SAVE_TASK_MEMORY_LIMIT);
    }

    public void addImage(final byte[] data, String title, long date, Location loc,
            int width, int height, int orientation, ExifInterface exif,
            OnMediaSavedListener l, ContentResolver resolver, int mode) {
        ...
        ImageSaveTask t = new ImageSaveTask(data, title, date,
                ...
                width, height, orientation, exif, resolver, l, mode);

        mMemoryUse += data.length;
        if (isQueueFull()) {
            onQueueFull();
            CameraUtil.P(DEBUG, TAG, "Cannot add image when the queue is full !");
        }
        t.execute();
    }
    ...
    public void addImage(final byte[] data, String title, long date, Location loc,
        ...
        addImage(data, title, date, loc, 0, 0, orientation, exif, l, resolver, mode);
    }
    public void addImage(final byte[] data, String title, Location loc,
        ...
        addImage(data, title, System.currentTimeMillis(), loc, width, height,
                orientation, exif, l, resolver, mode);
    }
    ...
    private class ImageSaveTask extends AsyncTask <Void, Void, Uri> {
        ...
        public ImageSaveTask(byte[] data, String title, long date, Location loc,
                             int width, int height, int orientation, ExifInterface exif,
                             ContentResolver resolver, OnMediaSavedListener listener, int mode) {
            this.data = data;
            this.title = title;
            this.date = date;
            this.loc = loc;
        ...
        protected Uri doInBackground(Void... v) {
            ...
            return Storage.addImage(
                    resolver, title, date, loc, orientation, exif, data, width, height, mode);
        }
        ...
        protected void onPostExecute(Uri uri) {
            ...
            if (mListener != null && mMemoryUse == 0) {
                mListener.onQueueEmpty();
            }
        }
    }
