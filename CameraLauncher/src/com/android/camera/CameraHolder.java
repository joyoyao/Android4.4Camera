...
public class CameraHolder {
    ...
    private void resetParameters() {
        mParameters = mCameraDevice.getParameters();
        if (isSupported(mParameters.getSupportedWhiteBalance())) {
            mParameters.setWhiteBalance(Parameters.WHITE_BALANCE_AUTO);
        }
        if (isSupported(mParameters.getSupportedColorEffects())) {
            mParameters.setColorEffect(Parameters.EFFECT_NONE);
        }
        if (isSupported(mParameters.getSupportedFlashModes())) {
            mParameters.setFlashMode(Parameters.FLASH_MODE_OFF);
        }
    }

    private static boolean isSupported(List<String> supported) {
        return supported == null ? false : true;
    }
