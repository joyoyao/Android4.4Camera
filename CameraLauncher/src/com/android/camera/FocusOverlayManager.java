...
public class FocusOverlayManager {
    ...
    public void onSingleTapUp(int x, int y) {
        if (!mInitialized || mState == STATE_FOCUSING_SNAP_ON_FINISH
                // SPRD: check state property, if current is focusing state then jump
                || mState == STATE_FOCUSING) return;
    ...
    public void resetTouchFocus() {
        ...
        // Initialize mFocusArea.
        if (mFocusAreaSupported) {
            mFocusArea = null;    // SPRD: add for multi-focus
    ...
    /* SPRD: porting multi-focus feature start @{ */
    private void executeFocusTapUp(
            boolean single, List<Tuple<Integer, Integer>> pointers) {
        int previewWidth = mPreviewRect.width();
        int previewHeight = mPreviewRect.height();
        ...
        if (pointers != null && pointers.size() > 0) {
            for (int i = 0, len = pointers.size(); i < len; i++) {
                ...
                // SPRD: fix bug 214940, protecting i out of mFocusArea's size
                int areaIndex = (i >= mFocusArea.size()) ? (mFocusArea.size() - 1) : i;

                calculateTapArea(x, y, 1f, ((Area) mFocusArea.get(areaIndex)).rect);
                calculateTapArea(x, y, 1.5f, ((Area) mMeteringArea.get(areaIndex)).rect);

            }

            // fixed bug 193147 start
            // because of the value of 'MaxNumMeteringAreas' is 1,so we must
            // change the list size to 1.
            int maxNumMeterAreaSize = mParameters.getMaxNumMeteringAreas();
            if (mMeteringArea != null
                    && mMeteringArea.size() > maxNumMeterAreaSize) {
                Iterator<Area> it = mMeteringArea.iterator();
                while (it.hasNext() && (it.next() != null)
                        && (mMeteringArea.size() > maxNumMeterAreaSize)) {
                    it.remove();
                }
            }
            // fixed bug 193147 end
        }
    }

    public void onMultiTapUp(List<Tuple<Integer, Integer>> pointers) {
        ...
        executeFocusTapUp(false, pointers);
        // Use margin to set the focus indicator to the touched area.
        mUI.setMultiFocusPosition(pointers);
    ...
    /* porting multi-focus feature end @} */
}
