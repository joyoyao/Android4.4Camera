    ...
    public static final class VideoData extends LocalMediaData {
        ...
        public MediaDetails getMediaDetails(Context context) {
            ...
            if (mWidth == 0 ||mHeight == 0) {
                mediaDetails = resetWidthAndHeight(mediaDetails);
            }
            return mediaDetails;
        }

        private MediaDetails resetWidthAndHeight(MediaDetails details) {
            int width = 0;
            int height = 0;
            ...
            String val = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH);
            if (mWidth == 0) {
                width = (val == null) ? 0 : Integer.parseInt(val);
            }
            val = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT);
            if (mHeight == 0) {
        ...
        private final class VideoBitmapLoadTask extends BitmapLoadTask {
            ...
            protected Bitmap doInBackground(Void... v) {
                ...
                try {
                    ...
                    Log.e(TAG, "IllegalArgumentException MediaMetadataRetriever.setDataSource() fail:"
                            + e.getMessage());
                } catch (RuntimeException ex) {
                    Log.e(TAG, "RuntimeException MediaMetadataRetriever.setDataSource() fail:"
                            + ex.getMessage());
                }
                retriever.release();
                return bitmap;
