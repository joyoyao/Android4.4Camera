...
public class WideAnglePanoramaModule
    ...
    //SPRD: FixBug256298 Panoramic camera memory is too large, change the resolution to reduce memory usage.
    ...
    public void init(CameraActivity activity, View parent) {
        ...
        // SPRD: Switch from video module need updat Adapter
        // Switch from photo modul don't need updat Adapter
        // mActivity.UpdatPath();
    }
    ...
    private boolean openCamera() {
        ...
        if (mCameraDevice == null) {
            mMainHandler.sendEmptyMessage(OPEN_CAMERA_FAIL);
            return false;
        }
    ...
    private void resetToPreviewIfPossible() {
        /** SPRD: fixbug 261883, Fix skipped hide review layout calls before getSurfaceTexture
         *  merge from 4.4.1_ri orig @{  */
         reset();
        /**  @}  */
    ...
    private Uri savePanorama(byte[] jpegData, int width, int height, int orientation) {
        if (jpegData != null) {
            ...
            String filepath = Storage.generateFilepath(filename,CameraUtil.MODE_CAMERA);
    ...
    public void onPauseBeforeSuper() {
        ...
        /* SPRD: unregisterReceiver broadcast Receiver  @{  */
        if (mReceiver != null) {
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        mUI.updateControlsRes(newConfig.orientation);
    ...
    public boolean onBackPressed() {
        ...
        if (mThreadRunning) {
            if (mPaused || mCameraTexture == null) return true;
    ...
    public void installIntentFilter() {
        /* SPRD: fix bug 267736 add BroadcastReceiver intent register @{  */
...
//        mUI.onFlashButtonClicked();
    ...
    public void onSettingTitleClicked(View v) {
        // nothing to do because no settings in this module
    }
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
                ...
                String storagePath  = util.getStorageByMode (CameraUtil.MODE_PANORAMA);
                if (storagePath == null || !storagePath.contains(externalPath))
                    return;
                if (mThreadRunning) {
                    if (mPaused || mCameraTexture == null) return;
