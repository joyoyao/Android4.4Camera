...
public class PieRenderer extends OverlayRenderer
    ...
    private boolean isMultiFocus = false;
    private List<Tuple<Integer, Integer>> mPointers;
    private List<Tuple<Integer, Integer>> mFocusTuple;
    private List<RectF> mCircles;
    private List<RectF> mDials;
    ...
    private void init(Context ctx) {
        ...
        /* SPRD: multi-focus feature start @{ */
        mPointers = new ArrayList<Tuple<Integer, Integer>>();
        mFocusTuple = new ArrayList<Tuple<Integer, Integer>>();
        mCircles = new ArrayList<RectF>();
        mDials = new ArrayList<RectF>();
        /* multi-focus feature end @} */
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mState != STATE_PIE) {
            //drawFocus(canvas);  // origin
            /* SPRD: multi-focus feature start @{ */
            if (!isMultiFocus) {
                Log.d(TAG, "draw single focus");
                drawFocus(canvas);
            } else {
                Log.d(TAG, "draw multi focus");
                drawMultiFocus(canvas);
            }
            /* multi-focus feature end @} */
        }
    ...
    /* SPRD: multi-focus feature dev start @{ */
    public void setMultiFocus(List<Tuple<Integer, Integer>> pointers) {
        isMultiFocus = true;
        // init
        mPointers = new ArrayList<Tuple<Integer, Integer>>();
        mFocusTuple = new ArrayList<Tuple<Integer, Integer>>();
        mCircles = new ArrayList<RectF>();
        mDials = new ArrayList<RectF>();//SPRD

        mOverlay.removeCallbacks(mDisappear);
        if (pointers != null && pointers.size() > 0) {
            for (int i = 0, len = pointers.size(); i < len; i++) {
                Tuple<Integer, Integer> location = pointers.get(i);
                int x = location.first, y = location.second;
                mPointers.add(location);
                mFocusTuple.add(location);
            }
            setCircle(pointers);
        }
    }
    ...
    /* SPRD: multi-focus feature dev start @{ */
    private void setCircle(List<Tuple<Integer, Integer>> pointers) {
        for (int i = 0, len = pointers.size(); i < len; i++) {
            Tuple<Integer, Integer> location = pointers.get(i);
            int cx = location.first, cy = location.second;
            mCircles.add(new RectF(cx - mCircleSize, cy - mCircleSize,
                    cx + mCircleSize, cy + mCircleSize));
            mDials.add(new RectF(cx - mCircleSize + mInnerOffset, cy - mCircleSize + mInnerOffset,
                            cx + mCircleSize - mInnerOffset, cy + mCircleSize - mInnerOffset));
        }
    }
    /* multi-focus feature end @} */
    ...
    /* SPRD: multi-focus feature start @{ */
    public void drawMultiFocus(Canvas canvas) {
        if (mBlockFocus) return;

        int x = 0, y = 0;
        if (mPointers != null && mPointers.size() > 0) {
            for (int i = 0, len = mPointers.size(); i < len; i++) {
                mFocusPaint.setStrokeWidth(mOuterStroke);
                Tuple<Integer, Integer> location = mPointers.get(i);
                x = location.first;
                y = location.second;
                canvas.drawCircle((float) x, (float) y, (float) mCircleSize, mFocusPaint);
                if (mState == STATE_PIE) return;
                int color = mFocusPaint.getColor();
                if (mState == STATE_FINISHING) {
                    mFocusPaint.setColor(mFocused ? mSuccessColor : mFailColor);
                ...
                mFocusPaint.setStrokeWidth(mInnerStroke);
                drawLine(canvas, mDialAngle, mFocusPaint, x, y);
                drawLine(canvas, mDialAngle + 45, mFocusPaint, x, y);
                drawLine(canvas, mDialAngle + 180, mFocusPaint, x, y);
                drawLine(canvas, mDialAngle + 225, mFocusPaint, x, y);
                canvas.save();
                // rotate the arc instead of its offset to better use framework's shape caching
                // inner circle radian
                canvas.rotate(mDialAngle, x, y);
                canvas.drawArc(mDials.get(i), 0, 45, false, mFocusPaint);
                canvas.drawArc(mDials.get(i), 180, 45, false, mFocusPaint);
    ...
    /* SPRD: multi-focus feature start @{ */
    private void drawLine(Canvas canvas, int angle, Paint p, int x, int y) {
        convertCart(angle, mCircleSize - mInnerOffset, mPoint1);
        convertCart(angle, mCircleSize - mInnerOffset + mInnerOffset / 3, mPoint2);
        canvas.drawLine(mPoint1.x + x, mPoint1.y + y,
                mPoint2.x + x, mPoint2.y + y, p);
    }
    /* multi-focus feature end @} */
